...
def train(model, train_set, val_set, test_set, loss_fn, opt, args, checkpoint_path, local_rank):
    # args setting
    ...
    for epoch_i in range(start_epoch, num_epochs + 1):
        ...
        # Collect the average loss from all ranks
        tensor_loss = torch.tensor(local_avg_train_loss).to(device)
        dist.reduce(tensor_loss, dst=0, op=dist.ReduceOp.SUM)
        
        if local_rank == 0:
            avg_train_loss = tensor_loss.item() / dist.get_world_size()  # Average the loss
