...
class TPCFDataset(Dataset):
    def __init__(self, input_file, label_file, target_labels):
        super().__init__()
...
def train(global_rank, local_rank, world_size, args):
    # load train and val
    train_ds = TPCFDataset(args['train_input'], args['train_label'], args['target_labels'])
    val_ds   = TPCFDataset(args['val_input'],   args['val_label'],   args['target_labels'])
    ...
    for epoch in range(args['epochs']):
        ...
        if global_rank == 0:
            ...
            logging.info(f"Saved curr model at {curr_path}")

            # Save best model so far
            if avg_val < best_val:
                best_val = avg_val
                best_model_state = model.module.state_dict()
                best_path = os.path.join(args['trial_dir'], 'best_checkpoint.pth')
                logging.info(f"Saved best model at {best_path}")
                torch.save(model.state_dict(), best_path)

    # After training, restore the best model
    if global_rank == 0 and best_model_state is not None:
        model.module.load_state_dict(best_model_state)

    return model.module if isinstance(model, DDP) else model, train_losses, val_losses, best_val

# ---------- Evaluation Function ----------
def evaluate(model, pred_filename, args):
    ds = TPCFDataset(args['test_input'], args['test_label'], args['target_labels'])
    ...
    state_dict = torch.load(best_model_path, map_location=f"cuda:{local_rank}")
